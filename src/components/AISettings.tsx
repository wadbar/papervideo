...
export default function AISettingsComponent() {
  const { providers, activeProviderType, updateProvider, setActiveProviderType, systemSettings, updateSystemSettings } = useSettingsStore();
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-8 space-y-12 mb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-surface border border-outline-variant/30 rounded-[2.5rem] p-8 space-y-6 relative overflow-hidden"
      >
        <div className="flex flex-col md:flex-row items-baseline sm:items-start gap-6">
          <div className="p-5 rounded-3xl bg-secondary/15 text-secondary">
            <Database className="w-7 h-7" />
          </div>
          <div className="space-y-2 flex-1">
            <h3 className="text-xl font-black text-on-surface flex items-center gap-3">
              Operational Resilience Sync
              {systemSettings.autoSync ? (
                <span className="text-[10px] font-black uppercase tracking-[0.2em] bg-secondary/10 text-secondary px-3 py-1 rounded-full border border-secondary/20 animate-pulse">
                  Auto-Sync Active [20s]
                </span>
              ) : (
                <span className="text-[10px] font-black uppercase tracking-[0.2em] bg-outline-variant/10 text-on-surface-variant px-3 py-1 rounded-full border border-outline-variant/20">
                  Manual Save Only
                </span>
              )}
            </h3>
            <p className="text-xs font-bold text-on-surface-variant opacity-60 uppercase tracking-widest">
              Periodic database reconciliation & local session recovery protocols
            </p>
            <p className="text-sm text-on-surface-variant leading-relaxed max-w-2xl pt-2">
              Forces recursive periodic reconciliation of the local canvas with the cloud database. Secures active workspace buffers against runtime memory reclamation, data leaks, or sudden tab dismissal.
            </p>
          </div>
        </div>
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            <div className="relative flex items-center justify-center">
              <input 
                ...
                onChange={(e) => updateSystemSettings({ autoSync: e.target.checked })} 
              ...
              <div className="absolute left-1 top-1 w-4 h-4 bg-on-surface-variant rounded-full peer-checked:translate-x-4 peer-checked:bg-on-secondary transition-transform shadow-sm"></div>
            </div>
            <span className="text-xs font-black uppercase tracking-widest text-on-surface group-hover:text-secondary transition-colors">
              Enable Auto-Sync Reconciliation
            </span>
          </label>
          <div className="text-[10px] font-mono font-bold text-secondary bg-secondary/10 border border-secondary/20 px-4 py-2 rounded-xl">
            {systemSettings.autoSync ? "RECONCILIATION STATE: POLLING_ACTIVE" : "RECONCILIATION STATE: SYSTEM_STANDBY"}
          </div>
        </div>
      </motion.div>
